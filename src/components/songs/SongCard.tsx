...
export function SongCard({ song, index }: { song: Song; index: number }) {
  ...
  return (
    ...
    >
      <span className="w-6 shrink-0 text-center text-xs tabular-nums text-white/40">
      ...
      >
        <p className={clsx("truncate text-sm", isCurrent ? "text-white" : "text-white/90")}>
          {song.title}
        </p>
        ...
        {!playable && <p className="mt-0.5 text-[10px] uppercase tracking-wide text-white/35">Needs confirmation</p>}
      </button>
      ...
      <div className="flex shrink-0 items-center gap-1">
        {playable && (
          <button
            ...
            className="flex h-11 w-11 items-center justify-center rounded-full text-white/40 opacity-0 transition-opacity hover:text-white group-hover:opacity-100 group-focus-within:opacity-100 sm:h-9 sm:w-9"
          >
        ...
        <button
          ...
          className={clsx(
            ...
            fav ? "text-[#ff6fa5]" : "text-white/40 hover:text-white"
          )}
        >
